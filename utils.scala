...
object Extractors {
  ...
  private class ExtractorsPrinter[Q <: Quotes & Singleton](using
      val quotes: Q
  ) { self =>
    import quotes.reflect._
    ...
    def visitSymbol(x: Symbol): this.type =
      ...
      else if x.isDefDef then
        this += "IsDefDefSymbol ( <" += x.fullName += "> ) "
      else if x.isValDef then
        this += "IsValDefSymbol ( <" += x.fullName += "> ) "
      else if x.isTypeDef then
        this += "IsTypeDefSymbol ( <" += x.fullName += "> ) "
...
object SourceCode {
  ...
  def showFlags(using
      Quotes
  )(flags: quotes.reflect.Flags)(syntaxHighlight: SyntaxHighlight): String = {
    import quotes.reflect._
    val flagList = List.newBuilder[String]
  ...
  private class SourceCodePrinter[Q <: Quotes & Singleton](
    ...
    def printTree(
        tree: Tree
    )(using elideThis: Option[Symbol] = None): this.type = tree match {
      ...
      case cdef @ ClassDef(
        ...
        if (flags.is(Flags.Final) && !flags.is(Flags.Module))
        ...
        else if (flags.is(Flags.Abstract))
      ...
      case vdef @ ValDef(name, tpt, rhs) =>
        ...
        if (vdef.symbol.flags.is(Flags.Mutable))
    ...
    private def printTargDef(
      ...
      argCons.rhs match {
        ...
        case rhs @ LambdaTypeTree(tparams, body) =>
          def printParam(t: Tree /*TypeTree | TypeBoundsTree*/ ): Unit =
            t match {
    ...
    private def printArgsDefs(
      ...
      inParens {
        if (argFlags.is(Flags.Implicit) && !argFlags.is(Flags.Given))
          this += "implicit "
          ...
          case vdef @ ValDef(`name`, _, _)
            ...
            if (!vdef.symbol.flags.is(Flags.Local)) {
              ...
              if (vdef.symbol.flags.is(Flags.Mutable))
                ...
                printedPrefix || !vdef.symbol.flags.is(Flags.CaseAccessor)
              ) this += highlightValDef("val ")
            }
        }
      end if
    ...
    private def printTypeOrBoundsTree(
        tpt: Tree
    )(using elideThis: Option[Symbol] = None): this.type = tpt match {
      case TypeBoundsTree(lo, hi) =>
        this += " _ >: "
        printTypeTree(lo)
        this += " <: "
    ...
    def printType(
        tpe: TypeRepr
    )(using elideThis: Option[Symbol] = None): this.type = tpe match {
    ...
    private def printDefAnnotations(
        ...
        names.get(sym).orElse {
          ...
          val name =
            if index == 1 then name0
              ...
              s"`$name0${index.toString.toCharArray.map { x =>
                  (x - '0' + '₀').toChar
                }.mkString}`"
          names(sym) = name
          Some(name)
